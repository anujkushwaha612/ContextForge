...
// Prices verified June 2026. Sources:
//   Anthropic: anthropic.com/pricing
//   OpenAI:    openai.com/api/pricing
//   Google:    ai.google.dev/gemini-api/docs/pricing
//   Others:    provider docs / pricepertoken.com
// ─────────────────────────────────────────────
...
const MODEL_PRICING = {
  ...
  // Legacy 3.x (still in use on some routes)
  ...
  // Codex (May 2025 — cloud agent, billed per-task but approximated per-token)
  "codex-mini": { input: 1.5, output: 6.0 },
  // Legacy (still routed by some proxies)
  "gpt-4-turbo": { input: 10.0, output: 30.0 },
  "gpt-4": { input: 30.0, output: 60.0 },

  // ── Google Gemini ────────────────────────────────────────────────────────
  // Gemini 2.5 family (current gen as of mid-2026)
  "gemini-3": { input: 2, output: 12.0 },
  "gemini-2.5-pro": { input: 1.25, output: 10.0 },
  "gemini-2.5-flash": { input: 0.15, output: 0.6 },
  "gemini-2.5-flash-lite": { input: 0.04, output: 0.15 },
  // Gemini 2.0 family (deprecated June 2026 but may still appear in logs)
  ...
  // ── Meta Llama (hosted — pricing via Together AI / Groq / Fireworks) ─────
  ...
  // Default fallback — conservative mid-tier estimate
...
function getPricing(modelName) {
  ...
  // 2. Starts-with match (handles versioned suffixes like -20250514)
  for (const [key, pricing] of Object.entries(MODEL_PRICING)) {
    if (key !== "default" && lower.startsWith(key)) return pricing;
  }

  // 3. Substring heuristics — most specific first to avoid mismatches
  ...
  // ── DeepSeek ──
  if (lower.includes("deepseek-prover"))
    return MODEL_PRICING["deepseek-prover-v2"];
  ...
  // ── Llama — Llama 4 before Llama 3 to avoid wrong match ──
  if (lower.includes("llama-4-behemoth"))
    return MODEL_PRICING["llama-4-behemoth"];
  if (lower.includes("llama-4-maverick"))
    return MODEL_PRICING["llama-4-maverick"];
  ...
  if (lower.includes("llama-3.1") || lower.includes("llama-3"))
  ...
  if (lower.includes("codestral-mamba"))
    return MODEL_PRICING["codestral-mamba"];
  ...
  if (lower.includes("mistral-small-3.1"))
    return MODEL_PRICING["mistral-small-3.1"];
  ...
  // ── AI21 Jamba ──
  if (lower.includes("jamba-1.5-large"))
    return MODEL_PRICING["jamba-1.5-large"];
  ...
  // ── Gemma (free on Groq, cheap on other hosts) ──
...
export class SavingsTracker {
  constructor({
    ...
    this._displaySessionInactivityMs =
      displaySessionInactivityMinutes * 60 * 1000;
   ...
   *   inputTokens  — tokens AFTER compression (what actually gets sent to the LLM)
   *   tokensSaved  — tokens removed by compression  (before − after)
   *   tokensBeforeCompression = tokensSaved + inputTokens
   */
  recordRequest({ model, inputTokens, tokensSaved, timestamp = null }) {
    ...
    if (deltaTokensSaved > 0) {
      this._state.history.push({
        ...
        compression_savings_usd: parseFloat(
          lifetime.compression_savings_usd.toFixed(6),
        ),
        total_input_tokens: lifetime.total_input_tokens,
        total_input_cost_usd: parseFloat(
          lifetime.total_input_cost_usd.toFixed(6),
        ),
      });
  ...
  /**
   * Generates a beautifully formatted ASCII table of all-time stats.
   *
   * Field definitions
   * ─────────────────
   *  total_input_tokens  → tokens AFTER compression (sent to LLM)
   *  tokens_saved        → tokens removed by compression
   *  tokensBeforeComp    → tokens_saved + total_input_tokens  (original prompt size)
   *  savings_percent     → tokens_saved / tokensBeforeComp × 100
   */
  getSummary() {
    const snap = this.snapshot();
    const lt = snap.lifetime;

    // tokensBeforeCompression = compressed tokens sent + tokens we cut out
    const tokensAfter = lt.total_input_tokens; // what the LLM saw
    const tokensBefore = lt.tokens_saved + lt.total_input_tokens; // original size

    const ratio =
      tokensBefore > 0
        ? ((lt.tokens_saved / tokensBefore) * 100).toFixed(1)
        : "0.0";
...
└────────────────────┴──────────────────────┘`;
  ...
  _displaySessionSnapshot() {
    ...
    if (
      !lastActivity ||
      utcNow() - lastActivity > this._displaySessionInactivityMs
    ) {
  ...
  async _saveAtomic() {
    ...
    const tmpFile = path.join(
      dir,
      `.cf_savings_${Date.now()}_${process.pid}.tmp`,
    );
  ...
  _loadState() {
    ...
    try {
      ...
      console.warn(
        `[SavingsTracker] Failed to load ${this._filePath}: ${err.message}`,
      );
      return defaultState;
    }
  }
  ...
  _sanitizeState(raw) {
    ...
    return {
      ...
      lifetime: {
        ...
        compression_savings_usd: Math.max(
          0,
          lifetime.compression_savings_usd || 0,
        ),
